...
public class FileUploadUtil {
	...
	public static void deleteDirectory(File directory) throws IOException {
		// Ottiene l'elenco dei file nella directory
		File[] files = directory.listFiles();
		// se ci sono file
		if (files != null) {
			// loop per controllare tutti i file nella lista
			for (File file : files) {
				if (file.isDirectory()) {
					// Se è una directory, chiama ricorsivamente la funzione per eliminare la
					// directory
			...
			// Se la directory è vuota, elimina la directory stessa
			directory.delete();
		}
		if (!directory.delete()) {
